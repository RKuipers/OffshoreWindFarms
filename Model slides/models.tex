...
1 \leq& \sum_{t = \dot{t}}^{\hat{t}} [ \sum_{t' = \dot{t}}^t f^I_{ait'} \cdot \sum_{t' = t}^{\hat{t}} s^I_{ajt'}]	&	\forall (i, j) \in IP, \forall a \in A		\\
...
0 =& \sum_{t = \dot{t}}^{t'} s^M_{act} \cdot \sum_{t = t'}^{\hat{t}} f^I_{ait}				&	\begin{aligned} \forall a \in A, \forall t' \in T, 	\\
...
d_a \geq& (f^M_{act''} + s^M_{act'} - 1) \cdot \sum_{t = t'}^{t''} \omega^M_{act} 				& 	\begin{aligned} \forall a \in A, \forall c \in C, \\
																	\forall t'' \geq t' + d_a, t', t'' \in T	\end{aligned}\\
N_{rp} \geq& \sum_{a\in A} \sum_{t' = \dot{t}}^t \sum_{t'' = t}^{\hat{t}} [ \sum_{i\in I} (s^I_{ait'} \cdot f^I_{ait''} \cdot \rho_{air}) + \sum_{c\in C} (s^M_{act'} \cdot f^M_{act''} \cdot \rho_{ar}) ]	& 	\forall r \in R, \forall p \in P, \forall t \in T_p 	\\
