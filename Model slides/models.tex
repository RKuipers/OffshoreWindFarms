...
b_{at} >&  \sum_{i \in M} [s_{ai(t - \lambda_a)} - s_{ait}]								&	\forall a \in A, \forall t \in T 			\\
...
o_{at} \leq& \frac{1}{2} \cdot (s_{ai_Nt} + \sum_{i \in M \cup \{i_N\}} [s_{ait} - s_{ai(t-\lambda_a)}]	& 	\forall a \in A, \forall t \in T 					\\
