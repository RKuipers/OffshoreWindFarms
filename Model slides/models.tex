...
\begin{aligned} M \cdot (a_{vij} + a_{vi'j}) + \\ 
d_{yi'} \cdot a_{vi'j} - 2M \end{aligned}				&\leq	 	s_i + s_{yi} - s_{i'} + s_{i'y}		& \forall y \in Y, \forall v \in V_y, \forall i, i' \in I, \forall j \in J				
\end{align}
...
\expl 58 \\ 
\bigskip
\expl Objective is to minimize costs of tasks being uncompleted (the +max bit is optional as it's about constants) \\ 
\expl 60 \\ 
\expl 61 \\ 
\expl 62 \\
\expl 63
