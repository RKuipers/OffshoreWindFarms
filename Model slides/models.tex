...
\begin{align}
d_c \geq& (f_{act''} + s_{act'} - 1) \cdot \sum_{t = t'}^{t''} \omega_{ct} 						& 	\begin{aligned} \forall a \in A, \forall c \in C, \\
																		\forall t'' \geq t' + d_a, t', t'' \in T	\end{aligned}\\
N_{rp} \geq& \sum_{a\in A} \sum_{c\in C} \sum_{t' = t_0}^t \sum_{t'' = t}^{t_N} s_{act'} \cdot f_{act''} \cdot \rho_{cr}	& 	\forall r \in R, \forall p \in P, \forall t \in T_p	\\
b_{at} >&  \sum_{c \in C} \sum_{t' = t - \lambda_a}^t - f_{act'}								&	\forall a \in A, \forall t \in T 	\\
...
1 \leq& \sum_{t = t_0}^{t_N} [ \sum_{t' = t_0}^t f^I_{ait'} \cdot \sum_{t' = t}^{t_N} s^I_{ajt'}]		&	\forall (i, j) \in IP, \forall a \in A		\\
...
0 =& \sum_{t = t_0}^{t'} s^M_{act} \cdot \sum_{t = t'}^{t_N} f^I_{ait}					&	\begin{aligned} \forall a \in A, \forall t' \in T, 	\\
