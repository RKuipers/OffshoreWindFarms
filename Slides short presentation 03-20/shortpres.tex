...
\footnotesize
\begin{equation}
	\text{maximize} \sum_{t \in T} [ DIS^t (O_t \cdot v_t - \sum_{r \in R} N_{rt} \cdot C_{rt}) ]
\end{equation}
...
\sum_{t \in T} s_{act}	\geq& \sum_{t \in T} s_{ac't}									&	\forall a \in A, \forall c + 1 = c', c, c' \in C^O		
\end{align} }
...
N_{rt} \geq& \sum_{a\in A} \sum_{c\in C} \sum_{t' = \dot{t}}^t \sum_{t'' = t}^{\hat{t}} s_{act'} \cdot f_{act''} \cdot \rho_{ar}	& 	\forall r \in R, \forall t \in T	\\
...
(11) Ensures if a cycle is maintained for the $c$th time there was a $c-1$th time (technically not needed?)	\\
(12) Ensures that between the starting and finish times of each cycle are enough timesteps with acceptable weather	\\
(13) Counts up the resources needed in a timestep summing up over all active tasks (an $s$ before and $f$ after $t$)	\\
