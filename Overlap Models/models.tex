...
\smalla
\expl The objective takes an average cost of all scenarios by counting up the cost per month. Each month has a cost for vessels chartered (by type), the energy lost due to planned maintenance, and the cost of failures (broken up in missed energy from unrepaired failures, repaired failures, planned repair tasks that fix older failures, and old ignored failures) \\
\expl For each vessel type the available time (based on charters and spare time from installation) minus the planned time the vessels are used (for planned maintenance) needs to be greater than the time spend fixing new failures. This LHS will never be negative, which enforces enough vessels are chartered to at least deal with all planned tasks. \\
...
\expl Three separate inequalities; (a) Each leftover failure is either still unsolved or repaired (b) the number of repairs does not exceed the amount of leftover failures (c) every leftover failure comes either from an old or a new failure \\
...
\begin{equation}
	\min_{\substack{s_{v,j} \in \mathbb{R}_{\geq 0} \\ 
	a_{v,i,j} \in \{0, 1\}}} 
	\sum_{y \in Y} \sum_{v \in V_y} \sum_{i \in I} \sum_{j \in J} c_i \cdot a_{v,i,j} \cdot (s_{v,j} + d_{y,i})
\end{equation}
subject to:
\begin{align}
\sum_{i \in I} a_{v,i,0}							&\leq	1							&\forall v \in V						\\
...
s_{v,j} + \sum_{i \in I} (a_{v,i,j} \cdot d_{y,i})			&\leq	T							&\forall y \in Y, \forall v \in V_y, \forall j \in J		\\
\sum_{j \in J} a_{v,i,j}							&=	a^{Inst}_{v,i}					& \forall v \in V, \forall i \in I^{Inst}  			\\
\sum_{j \in J} (s_{v_i,j} \cdot a_{v_i,i,j})				&=	s^{Inst}_i 						& \forall I \in I^{Inst} 						
\end{align}
