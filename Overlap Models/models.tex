...
\begin{equation}
	...
	M^P_{y, m}, M^R_{y, m, ir}}}
	\begin{aligned}
	\frac{1}{|S|} \cdot \sum_{\sigma \in S} ( \sum_{m \in M} [ 
	\sum_{y \in Y} (N_{y,m} \cdot c_{y,m}) +  
...
\item $M^P_{y, m}$: Minimum amount of vessels of type $y$ in month $m$ (based on assigned planned tasks)
\item $M^R_{y, m, ir}$: Minimum amount of vessels of type $y$ in month $m$ (based on assigned repair tasks of type $ir$)
\end{itemize}
