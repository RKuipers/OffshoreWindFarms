...
%NOTE: The problem here is neither stochastic model works (in the sense that it's nonsense unless the "task planning variables" are also scenario dependent, at which point you're just solving every scenario separately). Options I see are 1. don't include the model but simply describe where it went wrong, 2. include model as is but point out it makes no sense, 3. spend some time actually making that model with new variables and describe its value of scheduling the planned tasks. 
