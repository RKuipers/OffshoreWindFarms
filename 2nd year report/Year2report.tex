...
As the basis of my knowledge (of the literature, modeling, and implementation) is now fairly solid, and I have a basis of experience with the main methods used, the next steps will focus on producing new knowledge and attempting to answer the research questions posed in \Cref{ss:quest}. In the following months I will specifically focus on the first subquestion:

\sqa* 

This overlap of resources is the phenomenon already included in the combined model from \Cref{sss:comb}, and my aim is to create a model that focuses in on the overlap between two phases. At the moment I have a few possible approaches in mind to design such a model, and I will have to do research and experiments to determine which approach works best in practice. One of the key design decisions on which approaches can differ is whether the overlapping phases are scheduled by a single model, or by separate models (a decomposed approach). For example, if looking at the overlap between the installation and maintenance phases, we could choose to create a novel model to schedule this entire overlapped period, or we could use a dedicated installation scheduling model (from the literature) and create a novel model which takes this installation schedule as an input, and which schedules the maintenance tasks without affecting the installation tasks. This latter model would still be a novel approach to maintenance, as it has to deal with turbines coming online gradually. This new maintenance model could serve until installation is complete, after which an already existing maintenance model can be used to determine a maintenance strategy for the rest of the lifespan (or until decommission starts, after which a blended approach can be used again). 

Which of these approaches works best depends cannot be said yet, and will depend on the circumstances. Decomposed approaches generally reduce computational time, but allowing installation decisions to be affected might allow for more optimizations. Therefore the way I plan to research this is by first attempting a model that plans both installation and maintenance tasks, but if this turns out to be too computationally intensive the installation tasks can be fixed (at values determined by a separate model from the literature). The choice would be to fix the installation tasks rather than the maintenance ones as installation will be significantly more costly than the maintenance over this period; installation will use expensive specialized vessels for every turbine, while maintenance of that scale will not be necessary on most turbines in the first few years. After a model such as this is made for the overlap between installation and maintenance, a very similar model can likely be made for the overlap between decommission and maintenance. 

If this research direction proves fruitful, our goal is to put these findings the novel model(s) in a paper to be published. 

\subsection{Timeline}
The current goal is to get that first research question sufficiently answered in the next few months, ideally before March. After that there are the other two questions left:

\sqb*

\sqc*

Since these observe the same phenomenon, they can likely be researched through similar models. From March on, my focus will be on these two joint questions. Exactly which approach to take will have to be decided on later, but the topic appears to lend itself for a simulation approach. A key thing to focus on is how the failure rate of turbines changes over the course of their lifespan. Some research has already been found on this changing failure rate \cite{carroll2015offshore,papatzimos2019data}, which will be useful in my future research on this. My hope is that this aspect of the research can produce another paper. 

Once this part of the research is also complete, which I hope to be around summer 2021, I will have satisfying answers to each of my sub-questions, at which point I can write my thesis and answer the main research question
