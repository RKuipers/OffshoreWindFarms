...
W_xh = theano.shared(vals, 'W_xh')
vals = numpy.asarray( 
    rng.normal(loc=0.0, scale=0.1, size=(recurrent_size, recurrent_size)))  
W_output = theano.shared(vals, 'W_output')  


def rnn_step(x, h_prev, W_xh):  
    return h_prev + theano.tensor.tanh(theano.tensor.dot(W_xh, x)) 
...
updates = [  
    ...
    (W_output, W_output - .01 * theano.tensor.grad(cost, W_output)),  
