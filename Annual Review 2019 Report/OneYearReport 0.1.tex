...
\subsubsection{More general work on stocahstic scheduling} \label{sss:stoch}
Lorem Ipsum \\
...
%HERE I WILL MOSTLY SAY WHAT I CURRENTLY HAVE ON MY WAY TO A SIMULATION TOOL, AND WHAT I AM PLANNING TO DO WITH IT
Apart from getting caught up with the literature, over the past year I have also laid the foundations for some practical experiments. I have programmed the basics of an adaptable simulator from scratch. It is not yet operational and not very developed so far, but it can easily be expanded upon to fit future research. Currently it simulates different vessels that each have a schedule of tasks they work on. In my current implementation, task duration is deterministic and independent of the weather, but this will obviously be changed when an actual model is being tested. When a vessel cannot perform a certain task during the current weather, it will wait at a designated spot, but it more likely will end up waiting near the location of its next task. Currently the set of vessels is constant and the project is simply completed in the shortest possible timespan, but this can be changed depending on the model design. The size of a single time step can be adjusted to most reasonable values without problems. 

%Paragraph about implementation (c++ and matlab)
The simulator has been implemented in C++ as it seems the most fitting out of all the languages I am proficient with. A benefit of C++ is that is can easily be combined with Matlab. I am planning to implement the optimisation model in Matlab as it is very suited for such uses and this will hopefully lead to a well optimised implementation (in terms of computational speed). While I am not yet proficient with Matlab, I hope I can then let the optimisation model in Matlab and the simulation model in C++ work together in whichever way I try (as this is dependent on the model design). 

%Paragraph about its uses
%TODO: Point out goals of sim, like determine feasibility and analyse uncertainty
My goal for this simulator is for it to be usable as a tool in most of the likely research directions my PhD will go. The way it is currently set up the simulator is most suitable for fixed-task projects such as installation and decommission of the energy sites. However it is possible to adapt the simulator for other types of projects such as the maintenance of the site. In \Cref{s:concl} I will discuss various research directions; the simulator could be used for each of those directions. 
